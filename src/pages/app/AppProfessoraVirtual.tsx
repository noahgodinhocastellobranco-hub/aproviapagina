      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="flex flex-wrap gap-2 justify-center max-w-lg">
              {[
                ...
                <button
                  key={s}
                  onClick={() => setInput(s)}
                  className="text-xs px-3 py-2 rounded-full border hover:bg-muted transition-colors"
                >
                  {s}
                </button>
              ))}
        ...
        {isLoading && (
          <div className="flex justify-start">
            <Card className="p-4 bg-muted"><Loader2 className="w-5 h-5 animate-spin text-primary" /></Card>
          </div>
        )}
